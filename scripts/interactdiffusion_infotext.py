# https://github.com/Mikubill/sd-webui-controlnet/scripts/infotext.py
...
def serialize_unit(unit: interactdiffusion.InteractDiffusionData) -> str:
    log_value = {
        field_to_displaytext(field): getattr(unit, field)
        for field in vars(interactdiffusion.InteractDiffusionData()).keys()
        if field not in ("enabled") and getattr(unit, field) != -1
        # Note: exclude hidden slider values.
    }
    if not all(SEPERATOR not in str(v) and ":" not in str(v) for v in log_value.values()):
        logging.error(f"Unexpected tokens encountered:\n{log_value}")
        return ""
    
    return f"{SEPERATOR} ".join(f"{field}: {value}" for field, value in log_value.items())


def parse_unit(text: str) -> interactdiffusion.InteractDiffusionData:
    return interactdiffusion.InteractDiffusionData(
        enabled=True,
        **{
            displaytext_to_field(key): parse_value(value)
            for item in text.split(SEPERATOR)
...
class Infotext(object):
    ...
    def unit_prefix(unit_index: int) -> str:
        return f"InteractDiffusion {unit_index}"
    ...
    @staticmethod
    def write_infotext(
        units: List[interactdiffusion.InteractDiffusionData], p: StableDiffusionProcessing
    ):
        """Write infotext to `p`."""
    ...
    def on_infotext_pasted(infotext: str, results: dict) -> None:
        """Parse InteractDiffusion infotext string and write result to `results` dict."""
        updates = {}
        for k, v in results.items():
            if not k.startswith("InteractDiffusion"):
                continue

            assert isinstance(v, str), f"Expect string but got {v}. Key is {k}, Full item is {results}"
            try:
                for field, value in vars(parse_unit(v)).items():
                    ...
                    logging.debug(f"InfoText: Setting {component_locator} = {value}")
            except Exception:
                logging.warn(
